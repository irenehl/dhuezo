 ...
 */
const parseMarkdownFileCached = cache(
  async (
    filePath: string
  ): Promise<ParsedMarkdown<Record<string, unknown>>> => {
...
export async function parseMarkdownFile<TFrontmatter = Record<string, unknown>>(
  ...
  const parsed = await parseMarkdownFileCached(filePath)
  return parsed as ParsedMarkdown<TFrontmatter>
}

/**
 * Get all Markdown files in a directory
 */
export const getMarkdownFilesInDir = cache(async (dir: string): Promise<string[]> => {
  try {
